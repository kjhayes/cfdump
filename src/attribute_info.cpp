...
Attribute_info::Attribute_info(std::istream& istr, std::ostream& err = std::cerr){ReadFromBinaryStream(istr, err);}
Attribute_info::~Attribute_info() {if(info != nullptr) {delete[] info;}}

void Attribute_info::ReadFromBinaryStream(std::istream& istr, std::ostream& err = std::cerr) {
...
void Attribute_info::WriteJSON(std::ostream& ostr, iou::JSONFormatting formatting) const {
    ...
    if(info != nullptr){
        iou::JSON::WriteJSONString(ostr, "Data (UTF-8)", (char*)info, formatting);
    }
}
